...
4. [ ] 右栏输入问题 → DeepSeek 回答以**流式逐字**显示，视图**自动滚动跟随**到底。首次会拉取并缓存该论文 PDF 全文（稍慢）；输入框上方状态条显示「✓ 已读入全文」或「⚠ 正文较长，已读入首尾约 N 万字」（不再静默截断），正文加载中时发送禁用（防空正文提问）。
4b. [ ] 对话 v2 能力：① 生成时「发送」变「停止」，点击可中断；② 每条回答 hover 出「复制 / 重新生成 / 存为笔记」，重新生成不会让旧答案复现；③ 回答末尾出现 2-3 个**追问建议** chip，点击即问；④ 深思的思维链可折叠（💭 点击展开）；⑤ 输入框支持 Shift+Enter 换行；⑥ **切换论文对话不丢、重启应用仍在**（按论文持久化），「清空对话」一键重置。
4c. [ ] **引用定位**：让 AI 引用页码（如问「论文第二页讲了什么」）→ 回答里出现蓝色 `[页N]` chip → 点击 → 中栏自动切到「全文 PDF」并滚动到第 N 页、该页边框高亮闪烁 ~1.5s。AI 偶尔不标或标错页属可接受降级。
4d. [ ] **PDF 选字 + 问这段**：在「全文 PDF」里拖选一段文字（能选中、可 Cmd+C 复制）→ 选区旁浮出「✨ 问这段」→ 点击 → 右栏对话输入框注入「针对这段内容：『…』」、对话栏自动展开（若原本收起）→ 可直接发送或补充问题。
...
8. [ ] 右栏点快捷提问 chip（如「核心贡献」）→ 直接发问并流式回答；输入框草稿不被清空。
...
12. [ ] 点左上「🧠 知识库」→ 应用启动已静默建好全库索引（「索引状态」Tab 看统计）；「我的笔记」聚合全部笔记（论文标题+日期），按标签/关键词筛选，「打开论文 →」跳转，「删除」两步确认（3 秒未确认自动复位）；筛选状态关掉重开不丢。
13. [ ] 知识库提问（如「哪些论文讨论了 RLHF？」）→ 流式回答带 [来源N] 标注，编号与下方来源 chips 一一对应；点 chip **展开命中原文片段**核对，「打开论文 →」即时跳转；可连续追问（支持「那它…」式指代）；「存为笔记」把问答沉淀进笔记体系；对话历史关掉重开仍在，「清空对话」一键重置；问库里没有的内容会诚实说「没检索到」而非报错。
